...
public class LnAddressReceiverTests
{
    // LnAddress-shaped LUD-06 metadata: minSendable is 1 msat and the callback lives on a DIFFERENT host
    // (IBEX Hub) than the lightning-address domain.
    const string PayMeta =
        "{\"tag\":\"payRequest\",\"status\":\"OK\",\"callback\":\"{CB}\",\"minSendable\":1,\"maxSendable\":10000000000,\"commentAllowed\":140,\"metadata\":\"[[\\\"text/plain\\\",\\\"Pay alice\\\"]]\"}";

    // LnAddress-shaped LUD-21 verify URL (served by IBEX Hub).
    const string LnAddressVerify =
        "https://ibex.flashapp.me/pay/lnurl/verify/b8fd30308f0b3f5d36aeb503296134c14187ac927d6e0797652ce36798f900ec";
    ...
    static ResolvedLnAddress Resolved(string host) =>
        new(new Uri($"https://{host}/.well-known/lnurlp/alice"), $"alice@{host}", host);

    static LnAddressReceiver Rx(string host, FakeHttp http, Network? network = null) =>
    ...
    public void Preimage_validation_matches_sha256()
    {
        ...
        Assert.True(LnAddressReceiver.IsValidPreimage(preimage, hash));
        Assert.False(LnAddressReceiver.IsValidPreimage(preimage, new string('1', 64)));
        Assert.False(LnAddressReceiver.IsValidPreimage("xyz", hash));
        Assert.False(LnAddressReceiver.IsValidPreimage(null, hash));
    }
    ...
    {
        // PayMeta advertises minSendable of 1 msat (as LnAddress does); only the 1000 msat route is
        // mapped, so this also pins the probe's ≥1 sat clamp — without it the request would be
        // amount=1 and 404.
        var host = "yv.example";
        var cb = $"https://{host}/.well-known/lnurlp/alice/";
        var http = new FakeHttp()
            .Map($"https://{host}/.well-known/lnurlp/alice", PayMeta.Replace("{CB}", cb))
            .Map($"{cb}?amount=1000", $"{{\"pr\":\"lnbc1\",\"verify\":\"{LnAddressVerify}\"}}");
        var rx = Rx(host, http);
    ...
    public async Task CreateInvoice_rejects_amount_mismatch()
    {
        ...
        var http = new FakeHttp()
            ...
            .Map($"{cb}?amount=100000", $"{{\"pr\":\"{SpecBolt11}\",\"verify\":\"{LnAddressVerify}\"}}");
        var rx = Rx(host, http);
    ...
    [Fact]
    public async Task CreateInvoice_uses_cross_host_callback_and_tracks_verify_url()
    {
        // Pins the LnAddress callback quirk: the callback lives on a DIFFERENT host (IBEX Hub) than the
        // lightning-address domain; the amount query is appended cleanly and the LUD-21 verify URL
        // from the callback response is what gets tracked.
        var host = "ok.example";
        var cb = $"https://ibex.{host}/pay/lnurl/alice";
        var http = new FakeHttp()
            .Map($"https://{host}/.well-known/lnurlp/alice", PayMeta.Replace("{CB}", cb))
        ...
        Assert.Equal(LnAddressVerify, tracked.VerifyUrl);
    ...
    public async Task CreateInvoice_truncates_comment_to_commentAllowed()
    {
        ...
        var http = new FakeHttp()
            .Map($"https://{host}/.well-known/lnurlp/alice", PayMeta.Replace("{CB}", cb));
        var rx = Rx(host, http);
        var description = new string('x', 200); // over LnAddress's commentAllowed of 140
    ...
    {
        Assert.Equal("abc", LnAddressReceiver.TruncateByTextElements("abcdef", 3));
        Assert.Equal("abcdef", LnAddressReceiver.TruncateByTextElements("abcdef", 10));
        ...
        Assert.Equal("\U0001F600\U0001F601", LnAddressReceiver.TruncateByTextElements("\U0001F600\U0001F601\U0001F602", 2));
        // Combining sequence (e + U+0301) stays intact as one text element.
        Assert.Equal("e\u0301", LnAddressReceiver.TruncateByTextElements("e\u0301x", 1));
    }
