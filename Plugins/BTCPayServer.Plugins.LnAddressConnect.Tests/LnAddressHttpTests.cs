...
public class LnAddressHttpTests
{
    [Theory]
    ...
    {
        Assert.True(LnAddressHttp.IsSafeUrl(new Uri(url), out var reason));
        Assert.Null(reason);
    }
    ...
    {
        Assert.False(LnAddressHttp.IsSafeUrl(new Uri(url), out var reason));
    ...
    public void Blocks_private_and_reserved_addresses(string ip)
        => Assert.True(LnAddressHttp.IsBlockedAddress(IPAddress.Parse(ip)));
    ...
    public void Allows_public_addresses(string ip)
        => Assert.False(LnAddressHttp.IsBlockedAddress(IPAddress.Parse(ip)));
}
