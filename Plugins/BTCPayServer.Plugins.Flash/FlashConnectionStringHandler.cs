...
public class FlashConnectionStringHandler : ILightningConnectionStringHandler
{
    ...
    private static readonly ConcurrentDictionary<string, (ResolvedFlash Resolved, DateTimeOffset Expiry)> _cache = new();
    ...
    private static readonly ConcurrentDictionary<string, Lazy<Task<ResolvedFlash>>> _resolving = new();
    ...
    public FlashConnectionStringHandler(IHttpClientFactory httpClientFactory, ILoggerFactory loggerFactory,
    ...
    public ILightningClient? Create(string connectionString, Network network, out string? error)
    {
        ...
        {
            error = "The key 'ln-address' (your Flash Wallet Lightning address or username) is mandatory for flash connection strings";
            return null;
        }
        ...
        else
        {
            ...
            var flight = _resolving.GetOrAdd(normalized, key => new Lazy<Task<ResolvedFlash>>(
                () => FlashResolver.Resolve(key, http, CancellationToken.None),
        ...
        return new FlashLightningClient(resolved, network, http, _loggerFactory);
    }
    ...
    private void EnsurePersistedInvoicesLoaded()
    {
        ...
        lock (_loadLock)
        {
            ...
            try
            {
                new FlashPersistence(_settings).LoadAsync().GetAwaiter().GetResult();
                _loaded = true;
            }
            catch (Exception e)
            {
                ...
                _loggerFactory.CreateLogger(nameof(FlashConnectionStringHandler))
                    .LogWarning(e, "Failed to load persisted Flash tracked invoices; will retry");
            }
        }
    }
}
