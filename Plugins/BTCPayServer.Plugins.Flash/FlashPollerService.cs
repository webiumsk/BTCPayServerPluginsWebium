...
/// <summary>
/// The single shared verify poller for every Flash connection. One loop iterates all tracked
...
/// </summary>
public sealed class FlashPollerService : IHostedService
{
    ...
    private readonly ILogger<FlashPollerService> _logger;
    private readonly IHttpClientFactory _httpClientFactory;
    private readonly TimeSpan _interval;
    private readonly FlashPersistence? _persistence;
    ...
    public FlashPollerService(ILogger<FlashPollerService> logger, IHttpClientFactory httpClientFactory,
        ISettingsRepository settings)
        : this(logger, httpClientFactory, DefaultInterval, new FlashPersistence(settings)) { }

    internal FlashPollerService(ILogger<FlashPollerService> logger,
        IHttpClientFactory httpClientFactory, TimeSpan interval, FlashPersistence? persistence = null)
    ...
    public async Task StopAsync(CancellationToken _)
    {
        ...
        {
            try { await _persistence.SaveAsync(); } catch (Exception e) { _logger.LogDebug(e, "Final Flash persist failed"); }
        }
    }
            ...
            try
            {
                ...
                if (invoices.Length > 0)
                {
                    ...
                    if (PollOverride is null)
                    {
                        ...
                        http = _httpClientFactory.CreateClient(FlashHttp.ClientName);
                ...
                if (_persistence != null)
                {
                    ...
                    if (version != _lastPersistedVersion && now - _lastPersist > PersistThrottle)
                    {
                        ...
                        catch (Exception e) { _logger.LogDebug(e, "Failed to persist Flash tracked invoices"); }
                    }
                }
            }
            catch (OperationCanceledException) when (ct.IsCancellationRequested) { break; }
            catch (Exception e) { _logger.LogDebug(e, "Flash verify poll cycle error"); }
    ...
    private async Task PollOne(TrackedInvoice t, HttpClient? http, CancellationToken ct)
    {
        ...
        try
        {
            var inv = PollOverride is not null
                ...
                : await FlashReceiver.PollAndBuild(t, http!, ct);
        ...
        catch (Exception e)
        {
            ...
            _logger.LogDebug(e, "Error polling Flash verify for {Hash} (attempt {N})", t.PaymentHash, errors);
        }
    }
}
...
/// </summary>
public sealed class FlashListener : ILightningInvoiceListener
{
    ...
    public FlashListener(Func<TrackedInvoice, bool> isMine)
    {
