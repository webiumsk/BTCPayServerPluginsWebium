...
/// <summary>
/// The per-connection BTCPay Lightning backend for a Flash Wallet Lightning address. Strictly
/// receive-only: the keys live in the merchant's Flash app (Spark), so sending, balance and channel
/// operations are unsupported by design.
/// </summary>
public sealed class FlashLightningClient : IExtendedLightningClient
{
    private readonly ResolvedFlash _resolved;
    private readonly FlashReceiver _receiver;

    private const string ReceiveOnlyMsg =
        "Flash connections are receive-only — BTCPay only holds the merchant's Lightning address, no " +
        "account credentials. Sending, balance and channel operations are not supported.";

    public FlashLightningClient(ResolvedFlash resolved, Network network, HttpClient http, ILoggerFactory lf)
    {
        _resolved = resolved;
        _receiver = new FlashReceiver(resolved, network, http, lf.CreateLogger(nameof(FlashReceiver)));
    }
    ...
    public Task<LightningInvoice[]> ListInvoices(ListInvoicesParams request, CancellationToken cancellation = default)
    {
        ...
        var mine = TrackedInvoiceRegistry.All()
            ...
            .Select(t => new LightningInvoice
            {
                ...
                BOLT11 = t.Bolt11,
                // Same expiry logic as FlashReceiver.BuildInvoice, so both read paths agree for
    ...
    public Task<ILightningInvoiceListener> Listen(CancellationToken cancellation = default)
        => Task.FromResult<ILightningInvoiceListener>(
            new FlashListener(t => t.PayEndpoint == _resolved.PayEndpoint.ToString()));
