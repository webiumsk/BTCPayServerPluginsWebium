...
/// <summary>
/// Aligns BTCPay's served LNURL-pay parameters with Flash's when a store's BTC lightning backend is
/// a Flash Wallet lightning address.
///
/// Why this is required: for such a store the payable BOLT11 is minted by Flash's LNURL server (the
/// <see cref="FlashLightningClient"/> proxies it), and that invoice commits, via its BOLT11
/// <c>h</c> (description hash) tag, to <em>Flash's own</em> LNURL metadata. BTCPay by default serves
...
/// (e.g. Phoenix, Flash itself) refuse to pay. By mirroring Flash's metadata here the hashes match
/// and the payment succeeds. This also corrects the advertised min/max sendable to Flash's real limits.
///
/// Tradeoff: the payer's wallet then shows Flash's identity line ("Pay to user@flashapp.me")
/// rather than the store description. This is unavoidable because the description hash is committed
/// by Flash.
/// </summary>
public class FlashLnurlRequestFilter : PluginHookFilter<LNURLPayRequest>
{
    public override string Hook => "modify-lnurlp-request";

    private readonly PaymentMethodHandlerDictionary _handlers;
    private readonly IHttpClientFactory _httpClientFactory;
    private readonly ILogger<FlashLnurlRequestFilter> _logger;

    public FlashLnurlRequestFilter(
        PaymentMethodHandlerDictionary handlers,
        IHttpClientFactory httpClientFactory,
        ILogger<FlashLnurlRequestFilter> logger)
    {
        _handlers = handlers;
    ...
    public override async Task<LNURLPayRequest> Execute(LNURLPayRequest arg)
    {
        try
        {
            ...
            // Resolve the store's BTC lightning connection string and detect a Flash ln-address.
            ...
            if (!TryGetFlashLnAddress(connectionString, out var lnAddress))
                return arg;

            var (username, domain) = FlashResolver.ParseLightningAddress(lnAddress!);
            ...
            if (!FlashHttp.IsSafeUrl(metadataUri, out _))
                return arg;

            using var httpClient = _httpClientFactory.CreateClient(FlashHttp.ClientName);
            httpClient.Timeout = TimeSpan.FromSeconds(10);
            var json = await FetchMetadataCached(httpClient, metadataUri, CancellationToken.None);
            if (json is null)
                return arg;

            ApplyFlashParameters(arg, json);
            return arg;
        }
        catch (Exception e)
        {
            ...
            _logger.LogWarning(e, "Failed to align LNURL-pay parameters with Flash; leaving BTCPay defaults.");
            return arg;
        }
    }
    ...
    /// <summary>Detects a Flash connection string and extracts its lightning address (bare usernames
    /// expanded to the default domain). Mirrors <see cref="FlashConnectionStringHandler"/>.</summary>
    internal static bool TryGetFlashLnAddress(string? connectionString, out string? lnAddress)
    {
        lnAddress = null;
        ...
        lnAddress = FlashResolver.NormalizeAddress(lnAddress);
        return true;
    }

    /// <summary>
    /// Overwrites the served metadata with Flash's (so the served-metadata hash matches the invoice's
    /// committed description hash), narrows the sendable bounds to the intersection of BTCPay's and
    /// Flash's limits (only ever narrowed, never widened, so a fixed-amount invoice with min == max is
    /// preserved) and caps the allowed comment length to Flash's.
    /// </summary>
    internal static void ApplyFlashParameters(LNURLPayRequest arg, JObject flashMetadata)
    {
        ...
        // Compute the intersection of [BTCPay.Min, BTCPay.Max] and [Flash.Min, Flash.Max]. If the two
        // ranges are DISJOINT (Flash's min exceeds BTCPay's max, or vice versa) there is no valid
        // amount, so leave BTCPay's advertised bounds untouched rather than fabricating a fixed amount
        // Flash would reject anyway. The callback's own amount validation still rejects any
