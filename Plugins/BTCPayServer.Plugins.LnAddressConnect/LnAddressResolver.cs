...
public sealed record ResolvedLnAddress(Uri PayEndpoint, string LnAddress, string DisplayHost);
...
public static class LnAddressResolver
{
    ...
    {
        var parts = lnAddress.Trim().Split('@');
    ...
    /// <summary>
    /// Resolves a Lightning address to its LNURL-pay endpoint and validates it by fetching the
    /// metadata (an address whose server is unreachable or errors fails here, at config time).
    /// </summary>
    public static async Task<ResolvedLnAddress> Resolve(string lnAddress, HttpClient http, CancellationToken ct)
    {
        lnAddress = lnAddress.Trim();
        var endpoint = LNURL.LNURL.ExtractUriFromInternetIdentifier(lnAddress);
        if (!LnAddressHttp.IsSafeUrl(endpoint, out var reason))
