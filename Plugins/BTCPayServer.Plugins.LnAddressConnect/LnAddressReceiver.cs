...
/// </summary>
public sealed class LnAddressReceiver
{
    ...
    public LnAddressReceiver(ResolvedLnAddress resolved, Network network, HttpClient http, ILogger logger)
    ...
    {
        JObject meta;
        try { meta = await LnAddressResolver.GetJson(_http, _resolved.PayEndpoint, ct); }
        ...
        // Some wallets advertise minSendable of 1 msat; a sub-satoshi probe can be rejected by the
        ...
        cb.Query = q.ToString();
        if (!LnAddressHttp.IsSafeUrl(cb.Uri, out var cbReason))
            return $"The LNURL callback URL is not an allowed destination: {cbReason}.";

        JObject json;
        try { json = await LnAddressResolver.GetJson(_http, cb.Uri, ct); }
        ...
        if (!LnAddressHttp.IsSafeUrl(verifyUri, out var vReason))
        ...
        CreateInvoiceParams? p, CancellationToken ct)
    {
        ...
        var meta = await LnAddressResolver.GetJson(_http, _resolved.PayEndpoint, ct);
        var callback = meta["callback"]?.Value<string>()
        ...
        var json = await LnAddressResolver.GetJson(_http, cb.Uri, ct);
        ...
        if (!LnAddressHttp.IsSafeUrl(verifyUri, out var verifyReason))
    ...
    public static async Task<LightningInvoice?> PollAndBuild(TrackedInvoice t, HttpClient http, ILogger? logger, CancellationToken ct)
    {
        ...
        if (!Uri.TryCreate(t.VerifyUrl, UriKind.Absolute, out var verifyUri) ||
            !LnAddressHttp.IsSafeUrl(verifyUri, out _))
        {
