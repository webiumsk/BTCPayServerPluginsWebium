...
/// </summary>
public sealed class LnAddressPollerService : IHostedService
{
    ...
    private readonly ILogger<LnAddressPollerService> _logger;
    private readonly IHttpClientFactory _httpClientFactory;
    private readonly TimeSpan _interval;
    private readonly LnAddressPersistence? _persistence;
    ...
    public LnAddressPollerService(ILogger<LnAddressPollerService> logger, IHttpClientFactory httpClientFactory,
        ISettingsRepository settings)
        : this(logger, httpClientFactory, DefaultInterval, new LnAddressPersistence(settings)) { }

    internal LnAddressPollerService(ILogger<LnAddressPollerService> logger,
        IHttpClientFactory httpClientFactory, TimeSpan interval, LnAddressPersistence? persistence = null)
    ...
    public async Task StopAsync(CancellationToken _)
    {
        ...
        {
            try { await _persistence.SaveAsync(); } catch (Exception e) { _logger.LogDebug(e, "Final LnAddress persist failed"); }
        }
    }
            ...
            try
            {
                ...
                if (invoices.Length > 0)
                {
                    ...
                    if (PollOverride is null)
                    {
                        ...
                        http = _httpClientFactory.CreateClient(LnAddressHttp.ClientName);
                ...
                if (_persistence != null)
                {
                    ...
                    if (version != _lastPersistedVersion && now - _lastPersist > PersistThrottle)
                    {
                        ...
                        catch (Exception e) { _logger.LogDebug(e, "Failed to persist LnAddress tracked invoices"); }
                    }
                }
            }
            catch (OperationCanceledException) when (ct.IsCancellationRequested) { break; }
            catch (Exception e) { _logger.LogDebug(e, "LnAddress verify poll cycle error"); }
    ...
    private async Task PollOne(TrackedInvoice t, HttpClient? http, CancellationToken ct)
    {
        ...
        catch (Exception e)
        {
            ...
            _logger.LogDebug(e, "Error polling LnAddress verify for {Hash} (attempt {N})", t.PaymentHash, errors);
        }
    }
}
...
/// </summary>
public sealed class LnAddressListener : ILightningInvoiceListener
{
    ...
    public LnAddressListener(Func<TrackedInvoice, bool> isMine)
    {
