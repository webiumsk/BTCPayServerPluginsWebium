...
public class BlitzPollerTests
{
    ...
    public async Task Poller_publishes_settled_and_prunes()
    {
        ...
        BlitzPollerService.PollOverride = (ti, _) => Task.FromResult<LightningInvoice?>(
            ti.PaymentHash == hash
                ? new LightningInvoice { Id = ti.PaymentHash, PaymentHash = ti.PaymentHash, Status = LightningInvoiceStatus.Paid }
                : null);

        using var listener = new BlitzListener(ti => ti.PaymentHash == hash);
        var waiter = listener.WaitInvoice(TestContext.Current.CancellationToken);

        var poller = new BlitzPollerService(
            NullLogger<BlitzPollerService>.Instance, new SimpleHttpClientFactory(), TimeSpan.FromMilliseconds(20));
        ...
        finally
        {
            ...
            BlitzPollerService.PollOverride = null;
        }
    }
    ...
    public async Task Poller_settles_many_invoices_concurrently_without_races()
    {
        ...
        BlitzPollerService.PollOverride = (t, _) =>
        {
        ...
        var poller = new BlitzPollerService(
            NullLogger<BlitzPollerService>.Instance, new SimpleHttpClientFactory(), TimeSpan.FromMilliseconds(10));
        await poller.StartAsync(TestContext.Current.CancellationToken);
        try
        {
