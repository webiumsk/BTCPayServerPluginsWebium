...
public class BlitzPersistenceTests
{
    ...
    public async Task Save_writes_the_tracked_invoice_to_settings()
    {
        ...
        await new BlitzPersistence(settings).SaveAsync();

        var stored = await settings.GetSettingAsync<PersistedTrackedInvoices>(BlitzPersistence.SettingName);
