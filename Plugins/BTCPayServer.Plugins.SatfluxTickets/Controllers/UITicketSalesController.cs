...
using BTCPayServer.Plugins.SatfluxTickets.Controllers;
using BTCPayServer.Plugins.SatfluxTickets.Data;
using BTCPayServer.Plugins.SatfluxTickets.Helper;
...
using BTCPayServer.Plugins.SatfluxTickets.ViewModels.Models;
...
[Route("~/plugins/{storeId}/satflux-tickets/event/")]
        ...
        RaffleListClientProvider raffleListProvider) : Controller
{
    ...
    public async Task<IActionResult> List(string storeId, bool expired)
    {
        ...
        var apiDocUrl = Url.Action(nameof(SatfluxTicketsApiDocsController.Index), "SatfluxTicketsApiDocs", new { storeId }, Request.Scheme);
    ...
    public async Task<IActionResult> Settings(string storeId)
    {
        ...
        await using var ctx = dbContextFactory.CreateContext();
        var settings = ctx.SatfluxTicketsSettings.FirstOrDefault(s => s.StoreId == CurrentStore.Id);

        ViewData["StoreEmailSettingsConfigured"] = await emailService.IsEmailSettingsConfigured(CurrentStore.Id);
        var vm = new SatfluxTicketsSettingsViewModel
        {
            StoreId = CurrentStore.Id,
    ...
    [HttpPost("settings")]
    public async Task<IActionResult> Settings(string storeId, SatfluxTicketsSettingsViewModel vm)
    {
        if (string.IsNullOrEmpty(CurrentStore.Id))
            return NotFound();
        ...
        if (settings == null)
        {
            ctx.SatfluxTicketsSettings.Add(new SatfluxTicketsSetting
            {
                StoreId = CurrentStore.Id,
