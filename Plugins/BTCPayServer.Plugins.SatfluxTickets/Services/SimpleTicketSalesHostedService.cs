...
public class SimpleTicketSalesHostedService : EventHostedServiceBase, IPeriodicTask
{
    ...
    private readonly SatfluxTicketsRaffleBundleService _raffleBundleService;
    ...
    public SimpleTicketSalesHostedService(EmailService emailService,
        ...
        SatfluxTicketsRaffleBundleService raffleBundleService,
    ...
    public class PeriodProcessEvent
    {
        ...
        public SatfluxTicketsSetting Setting { get; set; }
    }
    ...
    public async Task Do(CancellationToken cancellationToken)
    {
        try
        {
            ...
            var storeSetings = db.SatfluxTicketsSettings.Where(s => s.EnableAutoReminders).ToList();
        ...
        {
            Logs.PayServer.LogInformation("Skipping task: SatfluxTickets table not created yet.");
        }
    }
        ...
        if (evt is PeriodProcessEvent sequentialExecute)
        {
            ...
            foreach (var ticketEvent in upcomingEvents)
            {
                ...
                try
                {
                    ...
                    if (!dispatchResult)
                    {
                        Logs.PayServer.LogWarning("SatfluxTickets: Reminder dispatch incomplete for event {EventId}", ticketEvent.Id);
                        continue;
                    }
                }
                catch (Exception ex)
                {
                    Logs.PayServer.LogWarning(ex, "SatfluxTickets: Failed sending reminders for event {EventId}", ticketEvent.Id);
                    continue;
                }
