...
public class FlashPersistenceTests
{
    ...
    public async Task Load_restores_non_expired_and_skips_expired()
    {
        ...
        await settings.UpdateSetting(snapshot, FlashPersistence.SettingName);

        await new FlashPersistence(settings).LoadAsync();
    ...
    public async Task Save_writes_the_tracked_invoice_to_settings()
    {
        ...
        await new FlashPersistence(settings).SaveAsync();

        var stored = await settings.GetSettingAsync<PersistedTrackedInvoices>(FlashPersistence.SettingName);
