...
public class FlashResolverTests
{
    // Flash-shaped LUD-06 metadata: callback is the lnurlp URL with a trailing slash.
    const string Pay =
    ...
    {
        Assert.Equal("alice@flashapp.me", FlashResolver.NormalizeAddress("alice"));
        Assert.Equal("alice@other.example", FlashResolver.NormalizeAddress("alice@other.example"));
        Assert.Equal(("alice", "flashapp.me"), FlashResolver.ParseLightningAddress(" alice "));
    }
    ...
    public async Task Full_address_resolves_to_lnurlp_endpoint()
    {
        ...
        var r = await FlashResolver.Resolve("alice@flashapp.me", http.Client(), TestContext.Current.CancellationToken);
    ...
    public async Task Non_payRequest_tag_is_rejected()
    {
        ...
        await Assert.ThrowsAsync<System.FormatException>(() =>
            FlashResolver.Resolve("bob", http.Client(), TestContext.Current.CancellationToken));
    }

    [Fact]
    public async Task Error_status_surfaces_reason()
    {
        // Flash returns this exact shape for unknown users.
        ...
        var ex = await Assert.ThrowsAsync<System.FormatException>(() =>
            FlashResolver.Resolve("ghost", http.Client(), TestContext.Current.CancellationToken));
