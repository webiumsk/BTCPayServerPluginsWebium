...
public class FlashReceiverTests
{
    // Flash-shaped LUD-06 metadata: minSendable is 1 msat and the callback lives on a DIFFERENT host
    ...
    // Flash-shaped LUD-21 verify URL (served by IBEX Hub).
    const string FlashVerify =
    ...
    static ResolvedFlash Resolved(string host) =>
        new(new Uri($"https://{host}/.well-known/lnurlp/alice"), $"alice@{host}", host);

    static FlashReceiver Rx(string host, FakeHttp http, Network? network = null) =>
    ...
    public void Preimage_validation_matches_sha256()
    {
        ...
        Assert.True(FlashReceiver.IsValidPreimage(preimage, hash));
        Assert.False(FlashReceiver.IsValidPreimage(preimage, new string('1', 64)));
        Assert.False(FlashReceiver.IsValidPreimage("xyz", hash));
        Assert.False(FlashReceiver.IsValidPreimage(null, hash));
    }
    ...
    {
        // PayMeta advertises minSendable of 1 msat (as Flash does); only the 1000 msat route is
        ...
        var http = new FakeHttp()
            .Map($"https://{host}/.well-known/lnurlp/alice", PayMeta.Replace("{CB}", cb))
            .Map($"{cb}?amount=1000", $"{{\"pr\":\"lnbc1\",\"verify\":\"{FlashVerify}\"}}");
        var rx = Rx(host, http);
    ...
    public async Task CreateInvoice_rejects_amount_mismatch()
    {
        ...
        var http = new FakeHttp()
            ...
            .Map($"{cb}?amount=100000", $"{{\"pr\":\"{SpecBolt11}\",\"verify\":\"{FlashVerify}\"}}");
        var rx = Rx(host, http);
    ...
    {
        // Pins the Flash callback quirk: the callback lives on a DIFFERENT host (IBEX Hub) than the
        ...
        var http = new FakeHttp()
            .Map($"https://{host}/.well-known/lnurlp/alice", PayMeta.Replace("{CB}", cb))
            .Map($"{cb}?amount=250000000", $"{{\"pr\":\"{SpecBolt11}\",\"verify\":\"{FlashVerify}\"}}");
        var rx = Rx(host, http);
        ...
        Assert.Equal(FlashVerify, tracked.VerifyUrl);
    ...
    public async Task CreateInvoice_truncates_comment_to_commentAllowed()
    {
        ...
        var http = new FakeHttp()
            .Map($"https://{host}/.well-known/lnurlp/alice", PayMeta.Replace("{CB}", cb));
        var rx = Rx(host, http);
        var description = new string('x', 200); // over Flash's commentAllowed of 140
