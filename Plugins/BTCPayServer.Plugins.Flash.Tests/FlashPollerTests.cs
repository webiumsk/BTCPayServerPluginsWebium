...
public class FlashPollerTests
{
    ...
    public async Task Poller_publishes_settled_and_prunes()
    {
        ...
        FlashPollerService.PollOverride = (ti, _) => Task.FromResult<LightningInvoice?>(
            ti.PaymentHash == hash
                ? new LightningInvoice { Id = ti.PaymentHash, PaymentHash = ti.PaymentHash, Status = LightningInvoiceStatus.Paid }
                : null);

        using var listener = new FlashListener(ti => ti.PaymentHash == hash);
        var waiter = listener.WaitInvoice(TestContext.Current.CancellationToken);

        var poller = new FlashPollerService(
            NullLogger<FlashPollerService>.Instance, new SimpleHttpClientFactory(), TimeSpan.FromMilliseconds(20));
        ...
        finally
        {
            ...
            FlashPollerService.PollOverride = null;
        }
    }
    ...
    public async Task Poller_settles_many_invoices_concurrently_without_races()
    {
        ...
        var inFlight = 0;
        var maxInFlight = 0;
        FlashPollerService.PollOverride = async (t, _) =>
        {
        ...
        var poller = new FlashPollerService(
            NullLogger<FlashPollerService>.Instance, new SimpleHttpClientFactory(), TimeSpan.FromMilliseconds(10));
        await poller.StartAsync(TestContext.Current.CancellationToken);
        try
        {
