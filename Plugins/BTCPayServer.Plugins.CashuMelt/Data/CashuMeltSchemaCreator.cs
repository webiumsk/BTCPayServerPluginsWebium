...
public static class CashuMeltSchemaCreator
{
    public static async Task EnsureSchemaAndTablesAsync(CashuMeltDbContext ctx, CancellationToken cancellationToken = default)
    {
        ...
        // 1.3.0 — NUT-08 change (also applied by EF migration AddNut08Change when EF runs)
        await ctx.Database.ExecuteSqlRawAsync(
            $@"ALTER TABLE ""{schema}"".""CashuMeltPaymentRequests"" ADD COLUMN IF NOT EXISTS ""BlankOutputsJson"" text",
            cancellationToken);
        await ctx.Database.ExecuteSqlRawAsync($@"
            CREATE TABLE IF NOT EXISTS ""{schema}"".""CashuMeltChangeProofs"" (
                ""Id"" bigint GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
                ""StoreId"" varchar(100) NOT NULL,
                ""MintUrl"" varchar(500) NOT NULL,
                ""Unit"" varchar(20),
                ""Amount"" bigint NOT NULL,
                ""KeysetId"" varchar(100) NOT NULL,
                ""Secret"" varchar(200) NOT NULL,
                ""C"" varchar(66) NOT NULL,
                ""State"" varchar(20) NOT NULL,
                ""SourceQuoteId"" varchar(100),
                ""CreatedAt"" timestamp with time zone NOT NULL,
                ""SweptAt"" timestamp with time zone,
                ""SweepReference"" varchar(200)
            )", cancellationToken);
        await ctx.Database.ExecuteSqlRawAsync($@"
            CREATE UNIQUE INDEX IF NOT EXISTS ""IX_CashuMeltChangeProofs_Secret""
            ON ""{schema}"".""CashuMeltChangeProofs"" (""Secret"")", cancellationToken);
        await ctx.Database.ExecuteSqlRawAsync($@"
            CREATE INDEX IF NOT EXISTS ""IX_CashuMeltChangeProofs_StoreId_State""
            ON ""{schema}"".""CashuMeltChangeProofs"" (""StoreId"", ""State"")", cancellationToken);
    }
}
