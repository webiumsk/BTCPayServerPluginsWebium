...
/// <summary>
/// The per-connection BTCPay Lightning backend for a Blitz Wallet Lightning address. Strictly
/// receive-only: the keys live in the merchant's Blitz app (Spark), so sending, balance and channel
/// operations are unsupported by design.
/// </summary>
public sealed class BlitzLightningClient : IExtendedLightningClient
{
    private readonly ResolvedBlitz _resolved;
    private readonly BlitzReceiver _receiver;

    private const string ReceiveOnlyMsg =
        "Blitz connections are receive-only — the keys stay in the merchant's Blitz Wallet app. " +
        "Sending, balance and channel operations are not supported.";

    public BlitzLightningClient(ResolvedBlitz resolved, Network network, HttpClient http, ILoggerFactory lf)
    {
        _resolved = resolved;
        _receiver = new BlitzReceiver(resolved, network, http, lf.CreateLogger(nameof(BlitzReceiver)));
    }
    ...
    public Task<ILightningInvoiceListener> Listen(CancellationToken cancellation = default)
        => Task.FromResult<ILightningInvoiceListener>(
            new BlitzListener(t => t.PayEndpoint == _resolved.PayEndpoint.ToString()));
    ...
    public string? DisplayName => "Blitz Wallet";
