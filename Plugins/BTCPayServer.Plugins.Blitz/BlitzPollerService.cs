...
/// <summary>
/// The single shared verify poller for every Blitz connection. One loop iterates all tracked
...
/// </summary>
public sealed class BlitzPollerService : IHostedService
{
    ...
    private readonly ILogger<BlitzPollerService> _logger;
    private readonly IHttpClientFactory _httpClientFactory;
    private readonly TimeSpan _interval;
    private readonly BlitzPersistence? _persistence;
    ...
    public BlitzPollerService(ILogger<BlitzPollerService> logger, IHttpClientFactory httpClientFactory,
        ISettingsRepository settings)
        : this(logger, httpClientFactory, DefaultInterval, new BlitzPersistence(settings)) { }

    internal BlitzPollerService(ILogger<BlitzPollerService> logger,
        IHttpClientFactory httpClientFactory, TimeSpan interval, BlitzPersistence? persistence = null)
    ...
    public async Task StopAsync(CancellationToken _)
    {
        ...
        {
            try { await _persistence.SaveAsync(); } catch (Exception e) { _logger.LogDebug(e, "Final Blitz persist failed"); }
        }
    }
            ...
            try
            {
                ...
                if (_persistence != null)
                {
                    ...
                    if (version != _lastPersistedVersion && now - _lastPersist > PersistThrottle)
                    {
                        ...
                        catch (Exception e) { _logger.LogDebug(e, "Failed to persist Blitz tracked invoices"); }
                    }
                }
            }
            catch (OperationCanceledException) when (ct.IsCancellationRequested) { break; }
            catch (Exception e) { _logger.LogDebug(e, "Blitz verify poll cycle error"); }
    ...
    private async Task PollOne(TrackedInvoice t, HttpClient? http, CancellationToken ct)
    {
        ...
        try
        {
            var inv = PollOverride is not null
                ...
                : await BlitzReceiver.PollAndBuild(t, http!, ct);
        ...
        catch (Exception e)
        {
            ...
            _logger.LogDebug(e, "Error polling Blitz verify for {Hash} (attempt {N})", t.PaymentHash, errors);
        }
    }
}
...
/// </summary>
public sealed class BlitzListener : ILightningInvoiceListener
{
    ...
    public BlitzListener(Func<TrackedInvoice, bool> isMine)
    {
