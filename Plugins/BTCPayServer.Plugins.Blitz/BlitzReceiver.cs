...
/// <summary>
/// The receive side: creates invoices via the Blitz LNURL-pay callback (capturing the LUD-21 verify
...
/// </summary>
public sealed class BlitzReceiver
{
    ...
    public BlitzReceiver(ResolvedBlitz resolved, Network network, HttpClient http, ILogger logger)
    ...
    private const string VerifyUnsupportedMessage =
        ...
        "required to detect payment settlement. Blitz Wallet addresses (user@blitzwalletapp.com) support it.";
    ...
    {
        JObject meta;
        try { meta = await BlitzResolver.GetJson(_http, _resolved.PayEndpoint, ct); }
        ...
        CreateInvoiceParams? p, CancellationToken ct)
    {
        ...
        var meta = await BlitzResolver.GetJson(_http, _resolved.PayEndpoint, ct);
        var callback = meta["callback"]?.Value<string>()
